(() => {
    ...
    /* ============================================================
       2. BLOODLINK PERSONAL DATA
    ============================================================ */

    let MY_BLOODLINK = null;

    let MY_BLOODLINK_READY = null;
    ...
    async function loadMyBloodLinkData() {
        ...
        try {
            ...
            /*
             * First try the existing API helper.
             */
    ...
    function latestRequest() {
        ...
        return MY_BLOODLINK.requests[0];
    ...
    function latestMatch() {
        ...
        return MY_BLOODLINK.matches[0];

    }


    /* ============================================================
       5. PERSONAL DASHBOARD ANSWERS
    ============================================================ */
    ...
    function dashboardAnswer(question) {
        ...
        if (
            /my blood group|my blood type|what blood group do i have|what is my blood group/.test(q)
        ) {

            if (data.bloodGroup) {

                return `Your registered blood group is ${data.bloodGroup}.`;

            }

            return "I couldn't find your blood group in your donor profile.";

        }


        if (
            /my address|my location|where am i registered|where is my location|where do i live/.test(q)
        ) {

            if (data.address) {

                return `Your registered location is ${data.address}.`;

            }

            return "I couldn't find a saved address in your profile.";

        }


        if (
            /am i available|my availability|available to donate|donor status/.test(q)
        ) {
        ...
        if (
            data.role === "DONOR" &&
            /how many matches|my matches|requests received|how many requests do i have/.test(q)
        ) {
        ...
        if (
            data.role === "DONOR" &&
            /which hospital|requesting hospital|hospital needs my blood|what hospital/.test(q) &&
            match
        ) {

            return `Your latest matched request is from ${matchRequest.hospital_name || "the hospital"}${match.distance_km != null ? `, approximately ${match.distance_km} km away` : ""}.`;
        ...
        if (
            data.role === "DONOR" &&
            /how far|distance|how many km|how far away/.test(q) &&
            match
        ) {
            ...
            if (
                ...
                return `Your latest matched hospital is approximately ${match.distance_km} km away.`;

            }

            return "I don't have a distance value for your latest match.";
        ...
        if (
            data.role === "DONOR" &&
            /how many units|units needed|units required/.test(q) &&
            match
        ) {

            return `The latest matched request requires ${matchRequest.units_required ?? "an unspecified number"} unit${matchRequest.units_required == 1 ? "" : "s"} of ${matchRequest.blood_group || data.bloodGroup || "blood"}.`;
        ...
        if (
            data.role === "DONOR" &&
            /blood group requested|what blood is needed|what blood group is needed|what group is needed/.test(q) &&
            match
        ) {

            return `The latest matched request is for ${matchRequest.blood_group || "an unspecified blood group"} blood.`;
        ...
        if (
            data.role === "DONOR" &&
            /match status|status of my match|accepted match|declined match|notified/.test(q) &&
            match
        ) {

            return `Your latest match status is ${match.status || "not available"}.`;
        ...
        if (
            data.role === "DONOR" &&
            /request status|is the request open|request still open/.test(q) &&
            match
        ) {

            return `The latest request status is ${matchRequest.status || "not available"}.`;
        ...
        if (
            data.role === "DONOR" &&
            /urgency|how urgent/.test(q) &&
            match
        ) {

            return `The latest matched request has ${matchRequest.urgency || "unspecified"} urgency.`;
        ...
        if (
            data.role === "DONOR" &&
            /when was the request|request date|date of request|created request/.test(q) &&
            match
        ) {

            return `The latest request was created on ${formatBLDate(matchRequest.created_at)}.`;
        ...
        if (
            data.role === "DONOR" &&
            /hospital address|address of hospital|where is the hospital/.test(q) &&
            match
        ) {

            return matchRequest.hospital_address
                ? `The hospital address is ${matchRequest.hospital_address}.`
                : "I don't have the hospital address for your latest match.";
        ...
        if (
            data.role === "REQUESTER" &&
            /how many requests|my requests|requests have i made|number of requests/.test(q)
        ) {
        ...
        if (
            data.role === "REQUESTER" &&
            /active request|active blood request|open request/.test(q)
        ) {
            ...
            return `Your active request is for ${activeRequest.blood_group || "unspecified"} blood at ${activeRequest.hospital_name || "the selected hospital"}, for ${activeRequest.units_required ?? "an unspecified number"} unit${activeRequest.units_required == 1 ? "" : "s"}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /my requested blood|blood group in my request|blood group of my request|what blood did i request/.test(q)
        ) {

            if (!request) {

                return "You don't have a blood request recorded yet.";

            }


            return `Your latest blood request is for ${request.blood_group || "an unspecified blood group"}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /which hospital|my hospital|hospital for my request|where is my request/.test(q) &&
            request
        ) {

            return `Your latest request is associated with ${request.hospital_name || "the selected hospital"}${request.hospital_address ? `, ${request.hospital_address}` : ""}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /how many units|units in my request|units requested|units required/.test(q) &&
            request
        ) {

            return `Your latest request requires ${request.units_required ?? "an unspecified number"} unit${request.units_required == 1 ? "" : "s"} of blood.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /fulfilled|how many units fulfilled|how much blood fulfilled/.test(q) &&
            request
        ) {

            return `Your latest request has ${request.units_fulfilled ?? 0} unit${request.units_fulfilled == 1 ? "" : "s"} fulfilled out of ${request.units_required ?? "the requested total"}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /request status|is my request open|status of my request/.test(q) &&
            request
        ) {

            return `Your latest request status is ${request.status || "not available"}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /urgency|how urgent is my request/.test(q) &&
            request
        ) {

            return `The urgency of your latest request is ${request.urgency || "not specified"}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /when did i create|when was my request|request creation date|date of my request/.test(q) &&
            request
        ) {

            return `Your latest request was created on ${formatBLDate(request.created_at)}.`;
        ...
        if (
            data.role === "REQUESTER" &&
            /appointment date|appointment time|when is my appointment|date of appointment/.test(q)
        ) {
            ...
            if (appointment) {
                ...
                return `Your appointment is scheduled for ${formatBLDate(appointment)}.`;

            }

            return "I don't see an appointment date or time stored in your current BloodLink request.";
        ...
        if (
            /unread notifications|unread notification|my notifications|notifications/.test(q)
        ) {
        ...
        if (
            /donation history|my donations|how many times have i donated|donations/.test(q)
        ) {
        ...
        if (
            /my dashboard|dashboard stats|my statistics|my stats|what is on my dashboard|show my dashboard/.test(q)
        ) {
            ...
            const fulfilledRequests =
                dashboard.fulfilled_requests ??
                0;


            return `Your BloodLink dashboard shows ${requests} total request${requests === 1 ? "" : "s"}, ${activeRequests} active and ${fulfilledRequests} fulfilled.`;
    ...
    function init() {
        ...
        /*
         * Start loading personal data immediately.
         */
...
`;
        ...
        async function sendMessage(
            ...
            if (!resultText) {
                ...
                resultText =
                    "I can help with blood donation, blood groups, iron, eligibility, hospital stress, anxiety, burnout, grief, recovery and your personal BloodLink dashboard. Try asking something like \"What is my blood group?\", \"How many matches do I have?\", or \"Which hospital needs my blood?\"";
